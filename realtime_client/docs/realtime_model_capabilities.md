...
|                        |                                                       | session.updated                   |
| User audio input       | conversation.item.create (send whole audio message)   | input_audio_buffer.speech_started |
|                        | input_audio_buffer.append (stream audio in chunks)    | input_audio_buffer.speech_stopped |
|                        | input_audio_buffer.commit (used when VAD is disabled) | input_audio_buffer.committed      |
|                        | response.create (used when VAD is disabled)           |                                   |
| Server audio output    | input_audio_buffer.clear (used when VAD is disabled)  | conversation.item.created         |
|                        |                                                       | response.created                  |
